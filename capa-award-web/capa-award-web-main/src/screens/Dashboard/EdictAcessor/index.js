...
export default function DashboardEdictAcessor(){  
    ...
    const columns = user?.isAdvisor || user?.isAdmin ? [
        ...
        { 
            ...
            renderCell: ({ row }) => !row?.id ? null : <>
                <ContentTableAction>
                    <Button secondary link nospace onClick={() => navigate(`activities/show/advisor/${ row.id }`)}>
                        Configurações
                    </Button>
                </ContentTableAction>
            </>
                            ...
                            <ActionsEnd>
                                {
                                    ...
                                    : 
                                        <ActionsContainerEnd>
                                            <Button primary nospace onClick={() => navigate(`activities/calendar/acessor`)}>Calendário</Button>
                                            <Button primary nospace onClick={() => history.goBack()}>Voltar</Button>
                                        </ActionsContainerEnd>
                                }
                            </ActionsEnd>
                            {
                                user?.isAdmin ? <DashboardTitle centred>Listagem de assessorias</DashboardTitle> :
                                user?.isAdvisor ? <DashboardTitle centred>Minhas assessorias</DashboardTitle> :
