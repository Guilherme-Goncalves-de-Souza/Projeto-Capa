import styled from 'styled-components'  

import {
    Animation
} from 'ui/styled'

export const DashboardTitle = styled.div.attrs({ 
})`            
    font-size: 22px;
    font-weight: bold;
    color: ${ props => props.theme.palette.colors.black };
    margin-bottom: 12px;
    ${
        props => props.centred ? `
            text-align: center;
        ` : ``
    }
`;

export const DashboardText = styled.div.attrs({ 
})`            
    font-size: 16px;
    line-height: 26px;
    color: ${ props => props.theme.palette.colors.black };
    ${
        props => props.centred ? `
            text-align: center;
        ` : ``
    }
`;

export const DashboardAnimation = styled(Animation).attrs({ 
    width: '100%',
    height: 420
})`             
`;

export const SearchContainer = styled.div.attrs({ 
})`
    display: flex;
    justify-content: center;
    align-items: center;
    gap: 20px;
    max-width: 540px;
    width: 100%;
    margin: 32px auto 0;
`;



export const EdictWrapper = styled.div.attrs({ 
})`
    width: 100%;
    display: flex;
    flex-wrap: wrap;
    gap: 64px 32px;
    margin: 32px 0;  
`;
export const EdictItem = styled.div.attrs({ 
})`
    width: 300px;
`;
export const EdictItemTitle = styled.div.attrs({ 
})`
    font-size: 18px;
    font-weight: bold;
    color: ${ props => props.theme.palette.colors.black };
    text-align: center;
    margin: 0 0 12px;
`;
export const EdictItemSubtitle = styled.div.attrs({ 
})`
    font-size: 18px;
    color: ${ props => props.theme.palette.colors.black };
    text-align: center;
`;
export const EdictItemContentImage = styled.div.attrs({ 
})`
    width: 300px;
    aspect-ratio: 1 / 1; 
    box-shadow: 0px 3px 6px ${ props => props.theme.palette.colors.shadow };

    display: flex;
    justify-content: center;
    align-items: center;
    margin: 20px 0;
`;
export const EdictItemImage = styled.img.attrs({ 
    width: 146,
    src: '/icons/file.svg'
})`
`;
export const EdictItemDescription = styled.div.attrs({ 
})`
    font-size: 18px;
    color: ${ props => props.theme.palette.colors.black };
    text-align: center;
    margin: 0 0 12px;
`;
export const EdictItemDate = styled.div.attrs({ 
})`
    font-size: 18px;
    color: ${ props => props.theme.palette.colors.black };
    text-align: center;
`;

export const FilterButton = styled.div.attrs({ 
})`
    width: 48px;
`;

export const FilterIcon = styled.img.attrs({ 
    src:'/icons/filter.svg'
})`
`;

export const FullPage = styled.div.attrs({ 
})`
    width: 100%;
`;


export const ActionsContainer = styled.div.attrs({ 
})`
    max-width: 240px;
    width: 100%;
    margin: 32px auto 0;
`;

export const ActionsContainerEnd = styled.div.attrs({ 
})`
    max-width: 240px;
    width: 100%; 
`;

export const ActionsEnd = styled.div.attrs({ 
})`
    display: flex;
    justify-content: flex-end;
    margin-bottom: 32px;
`;





export const ContentTitle = styled.div.attrs({ 
})`
    font-size: 18px;
    font-weight: bold;
    color: ${ props => props.theme.palette.colors.black };
    padding: 0 0 12px;
    margin: 32px 0;
    border-bottom: 1px solid ${ props => props.theme.palette.colors.shadow };
`;
export const ContentText = styled.div.attrs({ 
})`
    font-size: 18px;
    color: ${ props => props.theme.palette.colors.black }; 
    margin: 32px 0;
    ${
        props => props.small ? `
            margin: 12px 0;
        ` : ``
    }
    ${
        props => props.link ? `
            margin: 12px 0;
            color: ${ props => props.theme.palette.main.primary }; 
            cursor: pointer;
            &:hover{
                text-decoration: underline;
            }
        ` : ``
    }
`;


export const DashboardSpacer = styled.div.attrs({ 
})`
    height: 32px;
`;

export const DashboardContentCentred = styled.div.attrs({ 
})`
    display: flex;
    flex-direction: row;
    justify-content: center;
    align-items: center;
`;

export const DashboardLink = styled.div.attrs({ 
})`
    font-size: 16px;
    color: ${ props => props.theme.palette.secondary.main }; 
    ${
        props => props.primary ? `
            color: ${ props.theme.palette.primary.main }; 
            margin: 12px 0;
        ` : ``
    }
    cursor: pointer;
    &:hover{
        text-decoration: underline;
    }
`;
export const DashboardTextList = styled.div.attrs({ 
})`
    font-size: 16px;
    color: ${ props => props.theme.palette.colors.black };  
    margin: 12px 0;
`;
export const DashboardRemoveIcon = styled.img.attrs({ 
    src:'/icons/remove.svg'
})`
    margin: 0 0 0 12px;
    cursor: pointer;
    transition: all .3s ease;
    &:hover{
        transform: scale(1.1)
    }
`;




 

export const DashboardCalendar = styled.div.attrs({ 
})`
    border: 1px solid ${ props => props.theme.palette.colors.shadow };
    padding: 16px;
`;
export const DashboardCalendarTitle = styled.div.attrs({ 
})`
    font-size: 16px;
    font-weight: bold;
    text-align: center;
    color: ${ props => props.theme.palette.colors.black };  
`;
export const DashboardCalendarSelectors = styled.div.attrs({ 
})`
    display: flex;
    justify-content: flex-end;
    gap: 20px;
    margin-bottom: 20px;
`;
export const DashboardCalendarWrapper = styled.div.attrs({ 
})`
    display: flex;
    gap: 1%;
    flex-wrap: wrap;
`;
export const DashboardCalendarItemWeek = styled.div.attrs({ 
})`
    width: 13%;
    font-size: 14px;
    color: ${ props => props.theme.palette.colors.black }; 
    text-align: right;
    padding: 12px;
`;
export const DashboardCalendarItem = styled.div.attrs({ 
})`
    width: 13%;
    aspect-ratio: 1 / 1;
    padding: 12px 0;
    border-radius: 8px;
    border-top: 1px solid ${ props => props.theme.palette.colors.shadow };
    background: ${ props => props.today ? props.theme.palette.secondary.main : 'transparent' };
`;
export const DashboardCalendarItemDay = styled.div.attrs({ 
})`
    width: 30px;
    height: 30px;
    border-radius: 15px;
    display: flex;
    align-items: center;
    justify-content: center;
    margin: 0 12px 12px auto;
    border: 1px solid ${ props => props.theme.palette.colors.shadow };

    font-size: 14px;
    color: ${ props => props.today ? props.theme.palette.colors.white : props.theme.palette.colors.black };  
`;
export const DashboardCalendarItemSchedule = styled.div.attrs({ 
})`
    padding: 3px 6px;
    background: ${ props => props.theme.palette.primary.main };  
    color: ${ props => props.theme.palette.colors.white };  
    margin: 0 12px;
    border-radius: 6px;
    cursor: pointer;
    transition: all .3s ease;
    &:hover{
        transform: scale(1.05);
    }
`;