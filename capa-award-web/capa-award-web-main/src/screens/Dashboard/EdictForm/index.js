...
export default function DashboardEdictForm() {
  ...
  const changeKnowledgement = useCallback(
    (f) => {
          ...
          <Col>
            <FullPage>
              ...
              <EdictWrapper>
                <EdictItem>
                  <Input
                    placeholder="Nome autor principal"
                ...
                <EdictItem>
                  <Select
                    placeholder="CRediT"
                    options={credtis}
                    value={formValue("credit")}
                    onChange={(val) => changeForm(val, "credit")}
                  />
                </EdictItem>
                <EdictItem>
                  <Input
                    placeholder="ORCiD"
                    value={formValue("orcid")}
                    onChange={(e) => changeForm(e.target.value, "orcid")}
                  />
                </EdictItem>
                ...
                <EdictItem>
                  <Input
                    placeholder="PPG"
                    value={formValue("ppg")}
                    onChange={(e) => changeForm(e.target.value, "ppg")}
                  />
                </EdictItem>
                <EdictItem>
                  <Select
              ...
              {coauthors?.map((item, key) => (
                <>
                  ...
                  <EdictWrapper>
                    <EdictItem>
                      <Input
                        placeholder="Nome co autor"
                        value={formValue(`co_name${key}`)}
                        onChange={(e) => changeForm(e.target.value, `co_name${key}`)}
                      />
                    </EdictItem>
                    {/* <EdictItem>
                      <Select
                        ...
                        value={formValue(`co_institution${key}`)}
                        onChange={(val) => changeForm(val, `co_institution${key}`)}
                      />
                    </EdictItem> */}
                    <EdictItem>
                      <Select
                        placeholder="CRediT"
                        options={credtis}
                        value={formValue(`co_credit${key}`)}
                        onChange={(val) => changeForm(val, `co_credit${key}`)}
                      />
                    </EdictItem>
                    <EdictItem>
                      <Input
                        placeholder="ORCiD"
                        value={formValue(`co_orcid${key}`)}
                        onChange={(e) => changeForm(e.target.value, `co_orcid${key}`)}
                      />
                    </EdictItem>

                    <EdictItem>
                      <Select
                        placeholder="É orientador?"
                        options={booleanOptions}
                        value={formValue(`is_supervisor${key}`)}
                        onChange={(val) => changeForm(val, `is_supervisor${key}`)}
                      />
                    </EdictItem>
                    <EdictItem>
                      <EdictItem>
                    ...
                    <EdictItem>
                      <Input
                        placeholder="E-mail"
                        value={formValue(`co_email${key}`)}
                        onChange={(e) => changeForm(e.target.value, `co_email${key}`)}
                      />
                    </EdictItem>
                  </EdictWrapper>
                  <ActionsContainer>
                    <Button link primary onClick={() => removeCoAuthor(item.id)}>
                      Excluir co autor
                    </Button>
                  </ActionsContainer>
                </>
              ...
              <ContentTitle></ContentTitle>
              <ActionsContainer>
                <Button primary onClick={addCoAuthor}>
                  Adicionar autor
                </Button>
              </ActionsContainer>

              <ContentText>
                Faça o download de um modelo de Termo de ciência para obter a referência de
                preenchimento do seu modelo.
              </ContentText>
              <EdictItem>
                <Button
                  primary
                  onClick={() =>
                    model?.file?.url ? window.open(parseStrapiImage(model?.file?.url)) : null
                  }
                >
              ...
              {!knowledgementTerm?.name ? null : (
                <ContentFileUploaded>
                  <ContentText
                    link
                    onClick={() => window.open(`${parseStrapiImage(knowledgementTerm?.url)}`)}
                  >
              ...
              <ActionsContainer>
                <Button primary loading={loading} onClick={save}>
                  Prosseguir
                </Button>
              </ActionsContainer>
            </FullPage>
