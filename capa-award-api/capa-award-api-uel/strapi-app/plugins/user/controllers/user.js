...
module.exports = {
  ...
  update: async (ctx) => {
    try {
      ...
      const validadeDuplicate = ["username", "email"];
      ...
      if (ctx.is("multipart")) {
        ...
        if (!_.isEmpty(files)) {
          ...
          response = await strapi
            ...
            .update({ id: user.id }, Object.assign(data, images));
        }
      } else {
        response = await strapi
          .query("user", "users-permissions")
    ...
    try {
      ...
      if (
        ...
        const validPassword = await strapi.plugins[
          "users-permissions"
        ].services.user.validatePassword(data.currentPassword, user.password);
        ...
        const response = await strapi.plugins[
          "users-permissions"
        ].services.user.edit({ id: user.id }, updateData);
   ...
   * @return {Object}
   */
  fetchUserByEmail: async (ctx) => {
    console.log("executado");
    try {
      const { email } = ctx.query;
      console.log("email", email);
      if (!email) {
        return ctx.badRequest({ error: "Email is required" });
      }
