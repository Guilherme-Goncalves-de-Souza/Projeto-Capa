...
function SettingsPage() {
  ...
  var filteredMenu = useMemo(() => getSectionsToDisplay(menu), [menu]);
...
/*
    var filteredMenu2 = filteredMenu;
    for (var item in filteredMenu) {

        var newListMenu = [];
        for(var sub in filteredMenu[item].links) {
            if(filteredMenu[item].links[sub].name === "media-library") {
                console.log(filteredMenu[item].links[sub])
                filteredMenu[item].links[sub].isDisplayed = false;
            }
        }
    }
*/
              ...
              <StyledLeftMenu>
                {filteredMenu.map(item => {
                    for(var i in item.links) {
                       console.log(item.links[i].isDisplayed);
                       //item.links[i].isDisplayed = false;
                    }
